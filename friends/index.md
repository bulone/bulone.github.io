# 友情链接



---
{.awesome-hr}

## 基础信息

```yml
- nickname: 吐司气泡
  avatar: https://blog.toastbubble.top/avatar.webp
  url: https://blog.toastbubble.top
  description: 🍞这里有多的吐司快来品鉴
```

**评论格式如下:**

~点击右侧:(fa-regular fa-copy fa-fw me-1 ms-1):，直接修改里面对应的内容即可。~

````markdown {title=""}
```yaml
- nickname: 您的昵称
  avatar: 头像/网站Logo链接
  url: 网站地址
  description: 签名/网站描述
```
````

当然您也可以直接复制您网站对应的友链信息，无需按照此格式修改。不过要注意的是，请您使用代码块标记\`\`\`将您的内容进行包裹，避免链接过长导致页面布局变形，谢谢。

{{< admonition tip " 须知 " true >}}  
1. 仅接受个人非商业博客或原创内容网站，拒绝商业推广、采集站、违规内容（如政治敏感、色情、侵权等）。  
2.  人工审核，[会尽快回复]^(评论留邮箱的情况下)。  
3. 定期检查友链状态，如发现无法访问情况，进行半下架，后续如有恢复及时评论更新。  
4. 尽量拥有独立的域名，部分二级域名容易失效导致无法访问。
{{< /admonition >}}  


---

> 作者: bulone  
> URL: https://blog.toastbubble.top/friends/  

