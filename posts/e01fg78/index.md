...
3. 在命令行输入以下内容并回车，该命令意思是保数据进行卸载并重启手机；
   ...
   adb reboot
   ```
4. 重启完成后需要重新打开无线调试功能，等待手机链接无线调试后安装8.0.42版本，完成打开并随便选择一条聊天记录进行加载，否则后无升级版本可能或出现闪退现象。小程序也建议随机选择一个，软件弹窗提示升级并确定。
   ```title=8.0.42版本
   adb install 名称.apk
   ```
...
1. [微信相关文件，\_64结尾是64位版本有些系统32位安装不上](https://op.toastbubble.top/@s/ROlFWhnl)
